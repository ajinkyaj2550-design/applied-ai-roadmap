...
Return ONLY JSON. Every URL must be the actual page for that opportunity, not a search-results page. If a deadline is unknown, use an empty string. Keep each description concise and factual. Do not return any opportunity whose title/url/id matches this excluded list: ${JSON.stringify([...excludedIds].slice(0,120))} .`;
...
async function refreshOpportunities(req=null, excludedIds=new Set()) {
  ...
  let freshItems=[];
  try {
    freshItems=await geminiOpportunitySearch(req, excluded);
  } catch (geminiError) {
    const all=[];
    ...
    for(const x of all){
      ...
      const id=normalizeTitle(x.title)+"|"+x.domain; if(excluded.has(id)||excluded.has(String(x.url))||fresh.has(id))continue;
  ...
  old.forEach(x=>{if(x&&x.id&&!excluded.has(String(x.id))&&!excluded.has(String(x.url)))merged.set(String(x.id),x);});
  for(const x of freshItems){
    if(excluded.has(String(x.id))||excluded.has(String(x.url)))continue;
  ...
  const data={updatedAt:new Date().toISOString(),opportunities,stale:!freshItems.length};
...
function validateInterview(x,p,topics){
  // BUGFIX: we used to hard-reject whenever Gemini's echoed level/topic/difficulty
  // string did not match the selection byte-for-byte. Since the topic strings are
  // long mixed Marathi/English labels (e.g. "Python basics — variables, loops,
  // functions, OOP थोडक्यात"), the model almost never reproduced them exactly, so
  // every specific level/topic request failed validation and silently fell back
  // to the tiny local question bank -> the "same question keeps repeating" bug.
  // We only require the actual educational content to be present and in English;
  // the level/topic/difficulty labels are then forced to the requested values
  // below in makeInterview(), which is what the UI actually needs.
...
Avoid substantially similar questions: ${JSON.stringify(avoid)}.`;
  ...
  for(let attempt=0;attempt<2;attempt++){
    try{
      ...
      // BUGFIX: force the labels to exactly what the user selected instead of
      // rejecting good questions because Gemini phrased level/topic differently.
      if(level!=="all") x.level=level;
      if(topic!=="all") x.topic=topic;
      if(difficulty!=="auto") x.difficulty=difficulty;
...
QUESTION:\n${clean(p.question,5000)}\nCONTENT:\n${clean(p.content||p.answer,14000)}`;
  let last;
  for(const model of [GEMINI_TRANSLATE_MODEL,GEMINI_INTERVIEW_MODEL]){
    try{
      const r=await geminiGenerate({model,prompt,grounding:false,schema:TRANSLATION_SCHEMA,temperature:0.1,req});
      const x=parseJsonText(extractGeminiText(r));
      const out=clean(x.translation,18000);
      if(out)return out;
    }catch(e){last=e;}
  }
  throw last||new Error("Translation engine failed");
}
...
Return ONLY JSON.`;
  ...
  for(let attempt=0;attempt<2;attempt++){
    try{
      const r=await geminiGenerate({model:GEMINI_TEST_MODEL,prompt:prompt+ (attempt?"\nGenerate a different question than the previous attempt.":""),grounding:true,schema:TEST_SCHEMA,temperature:0.2,req});
      const x=parseJsonText(extractGeminiText(r));
  ...
  if(/[ऀ-ॿ]/.test(x.question+" "+x.options.join(" ")+" "+x.reason)) throw new Error("MCQ content must be English");
  // BUGFIX: same as the interview engine — force the labels to the requested
  // level/topic instead of rejecting a perfectly good MCQ because Gemini's
  // echoed level/topic string did not match character-for-character. That
  // mismatch was near-guaranteed for the long mixed Marathi/English topic
  // labels and was silently forcing every test question to the local fallback.
  if(level!=="all") x.level=level;
  if(topic!=="all") x.topic=topic;
      if(recent.some(q=>isSimilarQuestion(q,x.question))) throw new Error("Gemini returned a duplicate MCQ");
      x.sources=sourceListFromGemini(r); return x;
    }catch(e){last=e;}
  }
  throw last||new Error("Test engine failed");
}
...
const server=http.createServer(async(req,res)=>{
  ...
  try{
    if(req.method==="GET"&&u.pathname==="/api-status"){
      try{
        const r=await geminiGenerate({model:GEMINI_TRANSLATE_MODEL,prompt:"Reply with exactly the word OK.",grounding:false,schema:{type:"object",properties:{ok:{type:"string"}},required:["ok"],additionalProperties:false},temperature:0,req});
        return json(res,200,{ok:true,gemini:true,model:GEMINI_TRANSLATE_MODEL});
      }catch(e){
