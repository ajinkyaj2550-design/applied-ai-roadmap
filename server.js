...
async function refreshOpportunities() {
  const current=readData(), all=[]; let successful=0;
  ...
  for(const result of results){
    if(result.status==='fulfilled' && result.value.length){successful++;all.push(...result.value);}
  }
  ...
  for (const x of all) {
    ...
    const score=scoreItem(x.title,x.description,x.domain); if(score<30) continue;
  ...
  for(const x of fresh.values()){
    const prev=merged.get(String(x.id)); merged.set(String(x.id),prev?{...prev,...x,read:!!prev.read}:x);
  }
...
function similarityKey(v){return clean(v,600).toLowerCase().replace(/[^a-z0-9]+/g," ").replace(/\b(what|why|how|is|are|the|a|an|and|to|of|in|for|you|your)\b/g,"").replace(/\s+/g," ").trim();}
...
Avoid substantially similar questions: ${JSON.stringify(avoid)}.`;
  ...
  for(let attempt=0;attempt<2;attempt++){
    try{
      ...
      if(avoid.some(q=>similarityKey(q)===similarityKey(x.question))) throw new Error("Gemini returned a duplicate question");
