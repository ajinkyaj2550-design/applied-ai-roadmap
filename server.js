...
const GEMINI_OPPORTUNITY_MODEL = process.env.GEMINI_OPPORTUNITY_MODEL || "gemini-2.5-flash-lite";
...
const QUERIES = [
  ...
  "AI mentorship program 2026","AI jobs entry level India 2026","AI apprenticeship India 2026",
  "AI developer program 2026","cloud student program 2026","AI certification challenge 2026"
];
...
Find 10 to 15 REAL, currently relevant opportunities available to an Indian learner and aligned with an Applied AI Engineer roadmap: AI/ML/GenAI hackathons, competitions, internships, fellowships, scholarships, student ambassador programs, developer/student programs, apprenticeships, jobs, workshops, bootcamps, mentorships, challenges, cloud programs and relevant events. Prefer opportunities with a clear official or trusted source page and a current 2026 deadline/open status. Do not invent opportunities, dates or URLs.
...
Return ONLY JSON. Every URL must be the actual page for that opportunity, not a search-results page. If a deadline is unknown, use an empty string. Keep each description concise and factual. Do not return any opportunity whose title/url/id matches this excluded list: ${JSON.stringify([...excludedIds].slice(0,120))} .`;
  const r=await geminiGenerate({model:GEMINI_OPPORTUNITY_MODEL,prompt,grounding:true,schema:OPPORTUNITY_SCHEMA,temperature:0.15,req});
  ...
  const items=(Array.isArray(x.items)?x.items:[]).map((item,i)=>{
    ...
    if(!/hackathon|internship|fellowship|scholarship|competition|challenge|program|workshop|bootcamp|ambassador|apprenticeship|job|career|mentorship|student|developer|event|conference|certification/i.test(title+' '+description))return null;
...
const server=http.createServer(async(req,res)=>{
  ...
  try{
    if(req.method==="GET"&&u.pathname==="/api-status"){
      try{
        const r=await geminiGenerate({model:GEMINI_OPPORTUNITY_MODEL,prompt:"Reply with exactly the word OK.",grounding:false,schema:{type:"object",properties:{ok:{type:"string"}},required:["ok"],additionalProperties:false},temperature:0,req});
