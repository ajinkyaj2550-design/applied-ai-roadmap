...
async function refreshOpportunities(req=null, excludedIds=new Set()) {
  ...
  try {
    ...
    for(const x of all){
      ...
      if(excluded.has(id)||excluded.has(String(x.url)))continue;
  ...
  for(const x of old){
    if(!x||!x.id||excluded.has(String(x.id))||excluded.has(String(x.url)))continue;
    const d=x.deadline;
  ...
  const data={updatedAt:new Date().toISOString(),opportunities,stale:!freshItems.length,discovery:{count:opportunities.length,topCount:Math.min(TOP_OPPORTUNITIES,opportunities.length),source:freshItems.length?'gemini-search-grounded':'rss-fallback'}};
